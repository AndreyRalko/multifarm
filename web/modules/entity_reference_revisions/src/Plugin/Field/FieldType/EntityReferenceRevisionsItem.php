...
class EntityReferenceRevisionsItem extends EntityReferenceItem implements OptionsProviderInterface, PreconfiguredFieldUiOptionsInterface {
  ...
  public function setValue($values, $notify = TRUE) {
    if (isset($values) && !is_array($values)) {
      ...
      // Load the latest revision by default, if no revision has been passed-in
      // but we have a target ID.
      if (is_array($values) && !empty($values['target_id']) && !array_key_exists('target_revision_id', $values)) {
        $target_type_id = $this->getSetting('target_type');
        $storage = \Drupal::entityTypeManager()->getStorage($target_type_id);
        $entity = $storage->load($values['target_id']);
        assert($entity instanceof RevisionableInterface);
        $values['target_revision_id'] = $entity->getRevisionId();
      }
